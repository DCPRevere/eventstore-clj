...
(create-ns 'eventstore-clj.message.external)
(alias 'ext 'eventstore-clj.message.external)



;; Credentials

(create-ns 'eventstore-clj.message.credentials)
(alias 'creds 'eventstore-clj.message.credentials)

(s/def ::creds/login (s/and string? seq))
(s/def ::creds/password (s/and string? seq))
(s/def ::credentials (s/keys :req [::creds/login ::creds/password]))

(defn ->UserCredentials
  ([] (eventstore.UserCredentials/DefaultAdmin))
  ([{:keys [::creds/login ::creds/password]}]
...
(create-ns 'eventstore-clj.message.position)
(alias 'position 'eventstore-clj.message.position)
...
(create-ns 'eventstore-clj.message.stream)
(alias 'stream 'eventstore-clj.message.stream)
...
(defn stream-of [id]
  (match/match [id]
               [(:or "" nil)] {::stream/type ::stream/all, ::stream/system? true, ::stream/metadata? false}
...
(create-ns 'eventstore-clj.message.event)
(alias 'event 'eventstore-clj.message.event)

(s/def ::event/type (s/and string? seq))
(s/def ::event/id uuid?)

(create-ns 'eventstore-clj.message.event.data)
(alias 'e.data 'eventstore-clj.message.event.data)

(create-ns 'eventstore-clj.message.event.data.type)
(alias 'e.d.type 'eventstore-clj.message.event.data.type)
...
(defmethod ->Content ::e.d.type/json [m]
  (.apply (eventstore.Content$Json$.)
          (json/generate-string (::e.data/value m))))
...
(create-ns 'eventstore-clj.message.event.number)
(alias 'number 'eventstore-clj.message.event.number)
...
(create-ns 'eventstore-clj.message.version)
(alias 'version 'eventstore-clj.message.version)
...
(create-ns 'eventstore-clj.message.result)
(alias 'result 'eventstore-clj.message.result)

(s/def ::result/next-version ::version)
...
(create-ns 'eventstore-clj.message.direction)
(alias 'direction 'eventstore-clj.message.direction)
...
(defn ->IdentifyClient
  [{::keys [client-version connection-name]}]
...
;;  (->EventStream (stream-of "example"))
;;  (u/->scala-List (map ->EventData
;;                          [{:eventstore-clj.message.event/type "ExampleType"
;;                            :eventstore-clj.message.event/id (java.util.UUID/randomUUID)}]))
;;  (->ExpectedVersion ::version/any)
;;  true)
...
(create-ns 'eventstore-clj.message.transaction)
(alias 'transaction 'eventstore-clj.message.transaction)
...
(create-ns 'eventstore-clj.message.persistent)
(alias 'persistent 'eventstore-clj.message.persistent)
...
(create-ns 'eventstore-clj.message.persistent.nak)
(alias 'nak 'eventstore-clj.message.persistent.nak)
...
(create-ns 'eventstore-clj.message.settings)
(alias 'sett 'eventstore-clj.message.settings)

;; Scala durations must be smaller than 2^63 - 1 ns = 9223372036 s
(s/def ::duration (s/and nat-int? #(< % 9223372036)))
(s/def ::duration-!0 (s/and pos-int? #(< % 9223372036)))

(s/def ::sett/address string?)
(s/def ::sett/connection-name string?)
(s/def ::sett/connection-timeout ::duration)
(s/def ::sett/max-reconnections nat-int?)
(s/def ::sett/reconnection-delay-min ::duration-!0)
(s/def ::sett/reconnection-delay-max ::duration-!0)
(s/def ::sett/default-credentials ::credentials)
(s/def ::sett/hb-interval ::duration)
(s/def ::sett/hb-timeout ::duration)
(s/def ::sett/op-timeout ::duration)
...
(create-ns 'eventstore-clj.message.settings.overflow)
(alias 'of 'eventstore-clj.message.settings.overflow)

(s/def ::sett/on-overflow
  #{::of/drop-head ::of/drop-tail ::of/drop-buffer ::of/drop-new ::of/fail})

(create-ns 'eventstore-clj.message.settings.cluster)
(alias 'cl 'eventstore-clj.message.settings.cluster)
...
(s/def ::cl/dns-timeout pos-int?)
(s/def ::cl/max-discover-attempts nat-int?)
(s/def ::cl/attempt-interval nat-int?)
(s/def ::cl/discovery-interval nat-int?)
(s/def ::cl/gossip-timeout nat-int?)

(s/def ::uri-string
  (s/with-gen string? (fn [] (gen/fmap #(.toString %) (s/gen uri?)))))

(s/def ::sett/http ::uri-string)
...
(s/def ::sett/serialisation-parallelism pos-int?)
(s/def ::sett/serialisation-ordered? boolean?)

(s/def ::settings
  (s/keys :opt
          [::sett/address ::sett/connection-name ::sett/connection-timeout
           ::sett/max-reconnections ::sett/reconnection-delay-min ::sett/reconnection-delay-max
           ::sett/default-credentials ::sett/hb-interval ::sett/hb-timeout ::sett/op-timeout
           ;; ::sett/limit-op-attempts ::sett/limit-op-retries
           ::sett/keep-trying?
           ::sett/op-max-retries ::sett/perform-on-any-node ::sett/perform-on-master-only?
           ::sett/read-batch-size ::sett/buffer-size ::sett/on-overflow ::sett/cluster
           ::sett/http ::sett/serialisation-parallelism ::sett/serialisation-ordered?]))

(defn ->Settings
  [{::sett/keys [address connection-name connection-timeout max-reconnections
                reconnection-delay-min reconnection-delay-max default-credentials
                hb-interval hb-timeout op-timeout
                ;; limit-op-attempts
                limit-op-retries
                keep-trying? op-max-retries perform-on-any-node perform-on-master-only?
                read-batch-size buffer-size on-overflow cluster http
                serialisation-parallelism serialisation-ordered?]
    ::keys [resolve-links? master?]}]
  (let [b (eventstore.j.SettingsBuilder.)]
    (cond-> b
      (some? address) (.address address)
      (some? connection-name) (.connectionName connection-name)
      (some? connection-timeout) (.connectionTimeout connection-timeout)
      (some? max-reconnections) (.maxReconnections max-reconnections)
      (some? reconnection-delay-min) (.reconnectionDelayMin reconnection-delay-min)
      (some? reconnection-delay-max) (.reconnectionDelayMax reconnection-delay-max)
      (some? default-credentials) (.defaultCredentials (->UserCredentials default-credentials))
      (some? hb-interval) (.heartbeatInterval hb-interval)
      (some? hb-timeout) (.heartbeatTimeout hb-timeout)
...
(s/fdef ->Settings
  ...
  :ret #(instance? eventstore.Settings %))
...
(defn connect
  ([] (connect nil))
  ([settings]
   (let [eventstore-settings (or settings (eventstore.Settings/Default))
         system (akka.actor.ActorSystem/create)
         connection-actor (.actorOf system (eventstore.tcp.ConnectionActor/props eventstore-settings))
         esconn (eventstore.EsConnection. connection-actor system eventstore-settings)
         esconn-impl (eventstore.j.EsConnectionImpl. esconn eventstore-settings (.dispatcher system))]
     {::settings eventstore-settings
      ::actor-system system
      ::connection-actor connection-actor
      :eventstore/connection esconn
      :eventstore.j/connection esconn-impl})))
